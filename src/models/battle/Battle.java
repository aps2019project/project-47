...
//test
    ...
    private boolean ai_menu(Player player){
        ...
        boolean b = all_avsilble_works(playerNum,false,true);
        if (b)return true;
        return false;
    }
    private boolean human_menu(Player player){
        ...
        while (true){
            ...
            if (commandTxt.equals("help in game") || commandTxt.equals("14")){
                all_avsilble_works(playerNum,true,false);
                continue;
            }
            ...
            if (commandTxt.equals("m")){player.mana_rise(20);continue;}
            if (commandTxt.equals("play")||commandTxt.equals("17")){
                if(all_avsilble_works(playerNum,false,true))return true;
                continue;
            }
            if (commandTxt.equals("board +")){board.board_size_up();continue;}
            if (commandTxt.equals("board -")||commandTxt.equals("18")){board.board_size_down();continue;}
    ...
    public boolean all_avsilble_works(int playerNum,boolean print,boolean do_works){
        ...
        for (Minion minion:insiderForces){
            for (Location location:allLocations) {
                if (canMove(minion, location, false)) {
                    if (print) MyPrinter.purple(minion.getCardId() + " can move to cell by location : " + location.getX() + "," + location.getY());
                    if (do_works) {
